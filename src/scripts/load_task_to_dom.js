  ...
  Tasks.forEach((element, key) => {
    elementForDom += `<div class="card col-12 col-md-3 col-lg-2 col-xl-1 bg-dark text-white" style="width: 18rem">
          ...
          <ul class="list-group  list-group-flush">
            ...
            <li data-key="${key}" data-category="${category.title}" class="changeCompletion  list-group-item text-light  border-light bg-transparent">
          ...
          <button data-category="${category.title}" data-key="${key}" id="editTask" data-bs-toggle="modal" data-bs-target="#exampleModal" class=" editTask btn d-block btn-outline-warning   text-uppercase"><i class="bi bi-pencil-fill"></i></button>
           <button data-category="${category.title}" data-key="${key}" id="deleteTask"  class=" deleteTask  btn d-block btn-outline-warning   text-uppercase"><i class="bi bi-trash3-fill"></i></button>
          </div>
